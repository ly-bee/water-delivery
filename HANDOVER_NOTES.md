...
Roughly in order of how much they'd affect a real user:

1. ~~M-Pesa payment could only be started on a `PENDING` order, but orders could skip straight to
...
2. **M-Pesa is hard-coded to Safaricom's sandbox** (`MPESA_BASE_URL` in `mpesaService.js`). The
...
3. **The mobile app's backend URL is a hardcoded constant**, not configuration —
...
4. **No automated tests anywhere.** `hydroflow-backend`'s `npm test` is a no-op
...
5. **Email verification (web dashboard signup) will silently strand a user if Gmail isn't
...
6. **A handful of API endpoints exist but nothing in the UI calls them** — confirmed by checking for
...
7. **The M-Pesa routes are registered twice** (`/api/mpesa/*` and `/api/payments/mpesa/*` both point
...
8. **`drivers.rating` is a column that's never written to** — it's always whatever it was at driver
...
2. **Rotate every credential in it once you have it**, since the previous owner/developer will still
   ...
   - **M-Pesa (Safaricom Daraja)** — currently sandbox-only credentials, low risk, but if this moves
     to production you'll need to register your own business with Safaricom directly; ownership
     doesn't "transfer."
...
1. Rotate credentials as described above, before doing anything else with real user data.
2. Replace the stale `docs/` and `CONTRIBUTING.md` content (or delete it) so it stops contradicting
   reality.
3. Get *something* deployed (see [DEPLOYMENT.md](DEPLOYMENT.md)) — right now this only runs on a
   developer's laptop, which means it can't be handed to a real resident or driver to use yet.
4. Add at least a handful of backend tests around the order lifecycle and payment flow — those are
   the highest-stakes code paths and currently have zero automated coverage, so every change to them
   is verified by hand.
5. Decide whether the unused surface area (product catalog management, `drivers.rating`, the
